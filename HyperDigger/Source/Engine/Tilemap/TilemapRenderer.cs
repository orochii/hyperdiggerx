...
public class TilemapRenderer : IDisposable
{
    ...
    Dictionary<string, RenderedLevel> PrerenderedLevels { get; } = new Dictionary<string, RenderedLevel>();

    /// <summary> Gets or sets the levels identifier to layers Dictionary. </summary>
    Dictionary<string, Texture2D> TilemapCache { get; } = new Dictionary<string, Texture2D>();
    ...
    Texture2D[] RenderLayers(LDtkLevel level)
    {
        ...
        if (level.BgRelPath != null)
        {
            ...
            SpriteBatch.End();
            System.Console.WriteLine("Written BG Layer");
        }
        ...
        for (int i = 0; i < level.LayerInstances.Length; i++)
        {
            ...
            System.Console.WriteLine("Rendering layer: {0} {1}", i, layer._Identifier);
